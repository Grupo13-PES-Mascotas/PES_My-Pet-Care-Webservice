...
class StorageRestControllerTest {
    ...
    public void savePetImage() throws Exception {
        ...
        mockMvc.perform(put(BASE_URL + "/user/pets")
            .header("token", "my-token")
            .contentType(MediaType.APPLICATION_JSON)
            .content(json))
            .andExpect(status().isOk());
    ...
    public void downloadImage() throws Exception {
        ...
        assertEquals(downloadExpectedResult, result, "Should return the image as a base64 encoded string");
    }
    ...
    public void downloadAllPetsImages() throws Exception {
        ...
        MvcResult response = mockMvc.perform(get(BASE_URL + "/user/pets")
            .header("token", "my-token")
            .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk()).andReturn();
        String result = getContentAsString(response);
    ...
    public void shouldReturnErrorCode500WhenDatabaseFails() throws Exception {
        ...
        mockMvc.perform(get(BASE_URL + "/user/pets")
            .header("token", "my-token")
            .contentType(MediaType.APPLICATION_JSON))
